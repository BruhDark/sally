...
class Verification(commands.Cog):
    ...
    async def verify(self, ctx: discord.ApplicationContext):
        ...
        await ctx.respond(content=f":wave: To verify click the button below and follow the steps.\n-# {aesthetic.Emojis.info} By verifying you accept our [Terms of Service](<http://api.dark.is-a.dev/terms>) and [Privacy Policy](<http://api.dark.is-a.dev/privacy>)", view=VerifyView(ctx.author))

    @commands.slash_command(description="Get someones Roblox information", contexts={discord.InteractionContextType.guild})
    @commands.guild_only()
    @discord.option("user", discord.Member, description="The user to get the info from", default=None)
    @discord.option("roblox_id", str, description="The Roblox ID to look up for", default=None)
    ...
    async def check_lock(self, ctx: commands.Context, roblox_id: str):
        async with aiohttp.ClientSession() as session:
            async with session.get(f"http://api.dark.is-a.dev/lock/check-user?roblox_id={roblox_id}", headers={"Authorization": os.getenv("API_AUTHORIZATION_CODE")}) as resp:
