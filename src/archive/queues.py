...
class QueueCommands(commands.Cog):
    ...
    async def process_queue(self, process_channel: discord.TextChannel, queue_message: discord.Message, message_index: int):
        ...
        while True:
            try:
                item_msg = await process_channel.send("📫 Obteniendo un nuevo ticket...")
                item = queue.get_nowait()

            except asyncio.QueueEmpty:
                await item_msg.edit("⛔️ No hay tickets a procesar. Esperando un poco...")
            ...
            await queue_message.edit(embed=embed_q_msg)
            embed = discord.Embed(
                description="🚸 Esperando datos del usuario...", color=discord.Color.blurple())
            ...
            user_embed = discord.Embed(
                title="❗️ Es tu turno!", color=discord.Color.blurple())
            ...
            user_embed.add_field(
                name="🎫 Tu lugar en la fila era", value=str(item[0]))
            user_embed.add_field(name="⌛️ Tu ticket expirará en", value=utils.format_dt(
                datetime.utcnow()+timedelta(minutes=3), "R"))
    ...
    async def mensaje(self, ctx: discord.ApplicationContext, channel: discord.Option(discord.TextChannel, "El canal donde el mensaje de la fila será enviado")):
        ...
        embed.add_field(name="⚠️ Importante",
                        value=f"**Mantén tus Mensajes Directos (DMs) abiertos!** Cuando sea tu turno, {ctx.guild.me.mention} te enviará un mensaje para comenzar el proceso de venta de los tickets. Luego tus tickets serán enviados a revisión y se te enviará otro mensaje para notificar si fueron aprobados o denegados.")

        embed.add_field(name="🛃 Número de ticket siendo procesado ahora",
                        value="No hay ninguna fila activa!")
    ...
    async def crear(self, ctx, limit: int):
        ...
        embed = discord.Embed(
            description=f"✅ Creaste una fila con un límite de: {limit}", color=discord.Color.green())
    ...
    async def empezar(self, ctx, channel: discord.TextChannel):
        if self.queue is None:
            embed = discord.Embed(
                description="❌ No has creado una fila!", color=discord.Color.red())
            return await ctx.respond(embed=embed)

        embed = discord.Embed(
            description=f"✅ Hecho! He creado un procesador de fila en {channel.mention}", color=discord.Color.green())
        await ctx.respond(embed=embed)
        ...
        task = asyncio.create_task(
            self.process_queue(channel, msg, message_index))
    ...
    async def reiniciar(self, ctx, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            description=f"✅ Hecho! He reiniciado el proceso de la fila.", color=discord.Color.green())
        await ctx.respond(embed=embed)
